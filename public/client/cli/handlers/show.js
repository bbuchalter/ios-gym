 ...
 */
export function handleRender(session, args, action, templates) {
 ...
 */
function renderTemplate(template, session) {
    ...
    result = result.replace(/{#interfaces}([\s\S]*?){#endifaces_end}/g, (match, content) => {
        ...
        for (const [name, iface] of Object.entries(state.interfaces)) {
            ...
            line = line.replace(/{#if\s+l2mode\s+==\s+"routed"}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.l2mode === "routed" ? c : "";
            });
            line = line.replace(/{#if\s+l2mode\s+==\s+"access"}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.l2mode === "access" ? c : "";
            });
            line = line.replace(/{#if\s+l2mode\s+==\s+"trunk"}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.l2mode === "trunk" ? c : "";
            });
            line = line.replace(/{#if\s+accessVlan}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.accessVlan ? c : "";
            });
            line = line.replace(/{#if\s+trunkAllowed}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.trunkAllowed ? c : "";
            });
            line = line.replace(/{#if\s+ip}(.*?)(?={#endif|$)/g, (_m, c) => {
                return iface.ip ? c : "";
            });
            line = line.replace(/{#if\s+adminUp}(.*?)(?={#endif|$)/g, (_m, c) => {
