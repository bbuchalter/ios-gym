...
export class TabCompleter {
     ...
     */
    complete(line, cursorPos, mode, state) {
        ...
        for (const cmdDef of modeCommands) {
            const matches = this.getCompletionCandidates(tokens, isTrailingSpace, cmdDef, state);
     ...
     */
    getCompletionCandidates(tokens, isTrailingSpace, cmdDef, state) {
        const candidates = [];
        let tokenIndex = 0;
        let defIndex = 0;
        ...
        if (tokenIndex === tokens.length && isTrailingSpace && defIndex < cmdDef.tokens.length) {
            ...
            if (typeof nextToken === "string") {
                ...
                const argToken = nextToken;
                const argType = argToken.arg || argToken.optional_arg;
                if (argType) {
                    const suggestions = this.getSuggestionsForArgType(argType, state, cmdDef);
     ...
     */
    getSuggestionsForArgType(argType, state, cmdDef) {
        switch (argType) {
            case "IFNAME":
                // Suggest from state + defaults
                const ifnames = Object.keys(state.interfaces);
                const defaults = cmdDef.complete?.defaults || [
                    "g0/0", "g0/1", "g0/2", "fa0/1", "fa0/2", "vlan1"
                ];
                return [...new Set([...ifnames, ...defaults])].sort();
            case "VLAN_ID":
     ...
     */
    matchKeywordPrefix(userToken, keyword) {
